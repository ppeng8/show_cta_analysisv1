...
if add_sidebar == '统计检验-bootstrap_ttest':
    st.title('CTA策略情景分析')
    st.text_area('概要',
'1. 此网页展示主要包括两部分，统计检验-bootstrap_ttest和历史情景分析。\n\
2. 统计检验是为了验证策略在经济学假定的情境下策略的历史业绩收益率是有本质的不同，还是仅仅由于偶尔因素，报告结合实际背景，对时间序列采用bootstrap法 模拟N次历史情景，并通过箱线图和T统计量进行检验。\n\
3. 历史情景分析主要从两个维度出发，首先是对单个策略的分析，研究单个策略在不同场景下的表现差异,即整体_按照因子分类。其次，是对策略之间进行对比，研究同一情境下不同策略的表现；具体分为，单因子分析——在每一个场景，不同策略的实际sharp比率差异；单场景分析-策略对比，特定策略在不同因子下行（上行）时，对哪个因子更敏感；  单场景分析-因子对比，在特定因子下行（上行）时，所有策略中哪个策略表现最好'
                ,height=200)
...
4.11 长期动量时序，对利率的波动率、PPI预期差敏感。',height=450)
    ...
    st.text_area('说明',
'为了验证特定场景下，策略表现确实不同于所有场景在交替发生的正常经济环境中，而不是由于偶然因素或者随即发生的；报告对比特定场景下策略的收益率表现，通过两独立样本非参数检验两种场景下的收益率分布是否存在差异。考虑到样本区间数据有限，通过自助法采样来对比，每类场景的区间总长度在500天上下，每个场景连续出现的天数在20天上下；\
考虑到时间序列数据存在时序上的自相关性，并且为了避免时间上开始和结束两端的采样频率过低，报告采用stationary bootstrap方法（Dimitris N. Politis & Joseph P. Romano (1994)）。在全样本中采样1000次，得到1000个正常环境下的区间平均收益，特定场景样本同理，得到了该场景稳定发生时1000次平均收益，将这些不同环境下得到的收益率序列对比，通过非参检验验证是否不同场景收益率不同')
    st.text('某一因子上行的场景下、和正常环境下的 策略收益率分布是否不同',height=250)
